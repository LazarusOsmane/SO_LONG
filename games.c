...
/*   Updated: 2022/03/31 22:24:27 by engooh           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
#include "so_long.h"
...
void    file_to_image_xpm(t_all *all)
{
    ...
    if (!t)
        return ;
    t[0]->img = mlx_xpm_file_to_image(all->mlx, "image/ground5.xpm", &t[0]->x, &t[0]->y);
    t[1]->img = mlx_xpm_file_to_image(all->mlx, "image/ground.xpm", &t[1]->x, &t[1]->y);
    t[2]->img = mlx_xpm_file_to_image(all->mlx, "image/col3.xpm", &t[2]->x, &t[2]->y);
    t[3]->img = mlx_xpm_file_to_image(all->mlx, "image/col1.xpm", &t[3]->x, &t[3]->y);
    t[4]->img = mlx_xpm_file_to_image(all->mlx, "image/col2.xpm", &t[4]->x, &t[4]->y);
    t[5] = 0;
    all->xpm = t; 
}
